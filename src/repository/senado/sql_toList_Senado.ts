export const SQL_TO_LIST_SENADO = {
  CIRCUNCRIPCION_NACIONAL_SENADO:
    "SELECT dp.id_department,dp.name_department, vs.description_district,vs.description_role,sum(vs.number_votes) as votos, \
  ...
  CIRCUNCRIPCION_INDIGENA_SENADO:
    "SELECT dp.id_department,dp.name_department, vs.description_district,vs.description_role,sum(vs.number_votes) as votos, \
        ...
        GROUP BY dp.id_department,dp.name_department, vs.description_district, vs.description_role \
        ORDER BY dp.name_department ASC",
  LIST_ELEGIDOS_SENADO_NACIOAL:
    "SELECT distinct vt.candidate_name as candidate_name,vt.description_politicparty as description_politicparty,vt.description_role,vt.description_district,sum(vt.number_votes) as VOTOS, dep.name_department, t1.maximo as max_votes,\
  ...
  LISTA_ELEGIDOS_SENADO_INDIGENA:
    "SELECT distinct vt.candidate_name as candidate_name,vt.description_politicparty as description_politicparty,vt.description_role,vt.description_district,sum(vt.number_votes) as VOTOS, dep.name_department, t1.maximo as max_votes, \
    jsonb_object(array['idDepartment',dep.id_department,'name_department',dep.name_department]::TEXT[]) AS department \
    FROM t1 \
    INNER JOIN t2 ON t1.maximo = t2.votos and t1.candidate_name=t2.candidate_name \
